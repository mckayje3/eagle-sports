"""CFB Feature Analysis - Find optimal features for spread prediction."""
...
# Add points from game scores if points column is null
team_stats['points'] = team_stats.apply(
    lambda r: r['points'] if pd.notna(r['points']) else
    (r['home_score'] if r['is_home'] == 1 else r['away_score']), axis=1)
...
# ============================================================================
def build_features(games_df, team_stats_df, decay=0.94, min_games=3):
...
def walk_forward_test(df, features, train_min=100):
    ...
    for i in range(len(df_sorted)):
        ...
        results.append({
            ...
            'conf_game': test_row.get('conference_game', 0),
        })
    ...
    # Late weeks (10+)
    late = res_df[res_df['week'] >= 10]
    late_ats = (late['pred_cover'] == late['covered']).mean() if len(late) > 20 else np.nan

    # Conference games
    conf = res_df[res_df['conf_game'] == 1]
    conf_ats = (conf['pred_cover'] == conf['covered']).mean() if len(conf) > 20 else np.nan
...
# MAIN ANALYSIS
# ============================================================================
print("Building features with default decay=0.94...")
df = build_features(games, team_stats, decay=0.94, min_games=3)
...
for _, row in single_df.head(10).iterrows():
    ...
    conf = f"{row['conf_ats']:.1%}" if not pd.isna(row['conf_ats']) else "N/A"
    print(f"  {row['feature']:35} ATS: {row['ats']:.1%}  Early: {early}  Late: {late}  Conf: {conf}")
...
for name, features in combos.items():
    ...
    if result:
        ...
        conf = f"{result['conf_ats']:.1%}" if not pd.isna(result['conf_ats']) else "N/A"
        print(f"  {name:25} ATS: {result['ats']:.1%}  Early: {early}  Late: {late}  Conf: {conf}  ({len(features)} feat)")
    else:
...
decay_results = []
for decay in [0.88, 0.90, 0.92, 0.94, 0.96, 0.98]:
    print(f"\nTesting decay={decay}...")
...
best_decay = decay_df.iloc[0]['decay'] if len(decay_df) > 0 else 0.94

mingames_results = []
for min_g in [2, 3, 4, 5, 6]:
...
print()
print("=" * 70)
print("SUMMARY: RECOMMENDED CFB MODEL CONFIGURATION")
print("=" * 70)
...
BEST DECAY RATE:
  {best_decay_row['decay'] if best_decay_row is not None else 0.94}
...
# Save results
corr_df.to_csv('cfb_feature_correlations.csv', index=False)
combo_df.to_csv('cfb_combo_results.csv', index=False)
print("\nSaved: cfb_feature_correlations.csv, cfb_combo_results.csv")
