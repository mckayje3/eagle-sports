"""
NBA Odds Backfill from The Odds API
Fetches historical odds for NBA games
"""
import json
import time
import sqlite3
...
def backfill_nba_odds(season_year, start_month=10, end_month=6):
    """
    Backfill NBA odds for a season
    season_year: The year the season starts (e.g., 2023 for 2023-24)
    """
    print(f"\n{'='*80}")
    print(f"NBA {season_year}-{str(season_year+1)[-2:]} ODDS BACKFILL")
    print(f"{'='*80}\n")
    ...
    dates = []
    if start_month >= 10:
        # Oct-Dec of start year
        for month in range(start_month, 13):
            for day in [1, 8, 15, 22]:  # Sample dates each week
                try:
                    dates.append(datetime(season_year, month, day))
                except:
                    pass
    # Jan-Jun of next year
    for month in range(1, end_month + 1):
        for day in [1, 8, 15, 22]:
            try:
                dates.append(datetime(season_year + 1, month, day))
            except:
                pass

    print(f"Fetching odds for {len(dates)} sample dates")
    ...
    print(f"\n{'='*80}")
    print(f"NBA ODDS BACKFILL COMPLETE")
    print(f"{'='*80}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        backfill_nba_odds(season)
    else:
        # Default: backfill both seasons
        backfill_nba_odds(2023)  # 2023-24 season
        backfill_nba_odds(2024)  # 2024-25 season
