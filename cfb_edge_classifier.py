"""
CFB Edge Classifier - Neural network to identify profitable betting opportunities.
...
- Conference game indicator (higher predictability)
- Neutral site games (bowl games, etc.)
- Week of season
- Model agreement/disagreement patterns

Optimized parameters from feature analysis:
- DECAY = 0.88 (lower than NFL - more games provide stability)
- MIN_GAMES = 2
- Kitchen Sink features for Simple model
"""
...
# Model constants (must match training scripts) - OPTIMIZED FOR CFB
SIMPLE_DECAY = 0.88  # Lower decay for CFB - more games provide stability
ENHANCED_DECAY = 0.88
PREV_HALF_LIFE = 5.0
MIN_GAMES = 2
...
class DualModelPredictor:
    """Maintains state for both Simple and Enhanced CFB models."""

    def __init__(self):
        # Simple model - Kitchen Sink features (best performing for CFB)
        ...
        # Enhanced model - adds form/momentum and drive efficiency
    ...
    def _get_simple_stats(self, tid, season):
        """Get simple model stats - Kitchen Sink features."""
    ...
    def _get_dynamic_hca(self, home_id, season, neutral_site=False):
        ...
        if total == 0:
            return self.prev_hca.get(home_id, 3.5)  # CFB HCA ~3.5
    ...
    def extract_simple_spread_features(self, hid, aid, season, date, week, neutral_site=False):
        """Extract simple spread features - Kitchen Sink (best for CFB)."""
        ...
        # Kitchen Sink features (best combo from CFB analysis)
        return np.array([
    ...
    def extract_enhanced_spread_features(self, hid, aid, season, date, week,
        ...
        """Extract enhanced spread features - adds form and drive efficiency."""
        ...
        return np.array([
            ...
            min(week / 14.0, 1.0),  # CFB season progress
    ...
    def extract_enhanced_total_features(self, hid, aid, season, date, week):
        hs = self._get_enhanced_stats(hid, season)
        aws = self._get_enhanced_stats(aid, season)
        ...
        return np.array([
            ...
            min(hs['games'] / 10.0, 1.0),
            min(aws['games'] / 10.0, 1.0),
            (hs['games'] + aws['games']) / 24.0,
            1.0,
        ])

    def predict(self, hid, aid, season, date, week, **kwargs):
        neutral_site = kwargs.get('neutral_site', False)
        is_conf_game = kwargs.get('is_conf_game', False)

        simple_spread_feat = self.extract_simple_spread_features(
            hid, aid, season, date, week, neutral_site=neutral_site
        )
        simple_spread = None
        ...
        enhanced_spread_feat = self.extract_enhanced_spread_features(
            hid, aid, season, date, week, **kwargs
        )
        ...
        enhanced_total_feat = self.extract_enhanced_total_features(hid, aid, season, date, week)
        enhanced_total = None
    ...
    def update(self, tid, season, date, pf, pa, is_home, yards=None, pass_yards=None,
        ...
        # Simple stats - Kitchen Sink features
        ...
        if pd.notna(third_down_pct):
            ts['td_wts'] = [w * SIMPLE_DECAY for w in ts['td_wts']]
            ts['third_down_pct'].append(third_down_pct)
            ts['td_wts'].append(1.0)

        # Enhanced stats - adds form and drive efficiency
        ...
        if pd.notna(pass_yards):
            te['pass_wts'] = [w * ENHANCED_DECAY for w in te['pass_wts']]
            te['pass_yards'].append(pass_yards)
            te['pass_wts'].append(1.0)
        if pd.notna(rush_yards):
            te['rush_wts'] = [w * ENHANCED_DECAY for w in te['rush_wts']]
            te['rush_yards'].append(rush_yards)
            te['rush_wts'].append(1.0)
        if pd.notna(turnovers):
            te['to_wts'] = [w * ENHANCED_DECAY for w in te['to_wts']]
            te['turnovers'].append(turnovers)
            te['to_wts'].append(1.0)
        if pd.notna(first_downs):
            te['fd_wts'] = [w * ENHANCED_DECAY for w in te['fd_wts']]
            te['first_downs'].append(first_downs)
            te['fd_wts'].append(1.0)
        if pd.notna(ppd):
    ...
    def train_models(self):
        if len(self.simple_spread_X) >= 100:  # CFB has more games
...
class EdgeClassifier(nn.Module):
    """Neural network for classifying CFB betting opportunities."""
...
def generate_training_data():
    ...
    print("=" * 70)
    print("GENERATING CFB TRAINING DATA (Dual Model)")
    print("=" * 70)
    ...
    for season in seasons:
        ...
        for _, g in season_games.iterrows():
            ...
            sfeat = predictor.extract_simple_spread_features(
            ...
            etfeat = predictor.extract_enhanced_total_features(hid, aid, season, g['date'], week)
            ...
            if all(preds[k] is not None for k in
                ...
                if pd.notna(vegas_spread) and pd.notna(vegas_total):
                    ...
                    training_data.append({
                        ...
                        'big_favorite': 1 if abs(vegas_spread) > 14 else 0,  # CFB has bigger spreads
                        ...
                        'away_rest': ar,
                        'rest_diff': hr - ar,
                        # CFB-specific context
                        'conference_game': 1 if is_conf else 0,
                        'neutral_site': 1 if neutral else 0,
                        'week_normalized': min(week / 14.0, 1.0),
                        'early_season': 1 if week <= 4 else 0,
                        'mid_season': 1 if 5 <= week <= 10 else 0,
                        'late_season': 1 if week >= 11 else 0,
                        # Targets
    ...
    df.to_csv('cfb_edge_training_data.csv', index=False)
    print("Saved to cfb_edge_training_data.csv")

    return df


def analyze_patterns(df: pd.DataFrame):
    """Analyze CFB betting patterns."""
    print("\n" + "=" * 70)
    print("CFB SPREAD PATTERN ANALYSIS")
    print("=" * 70)
    ...
    print("\n5. NEUTRAL SITE:")
    for ns in [0, 1]:
        mask = df['neutral_site'] == ns
        n = mask.sum()
        if n > 0:
            wr = df.loc[mask, 'spread_cover'].mean() * 100
            label = "Neutral" if ns == 1 else "Home/Away"
            print(f"  {label}: {n} games, WR: {wr:.1f}%")


def train_edge_classifier(df: pd.DataFrame):
    """Train the CFB edge classifier."""
    print("\n" + "=" * 70)
    print("TRAINING CFB EDGE CLASSIFIER (SPREAD ONLY)")
    print("=" * 70)
    ...
    if '-' in best['record']:
        parts = best['record'].split('-')
        if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
            total = int(parts[0]) + int(parts[1])
            if total > 0:
                print(f"  Record: {best['record']} ({int(parts[0]) / total * 100:.1f}%)")
    ...
    }, MODEL_DIR / 'cfb_edge_classifier.pt')
    print(f"\nModel saved to {MODEL_DIR / 'cfb_edge_classifier.pt'}")
