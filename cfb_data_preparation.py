"""
CFB Data Preparation for Deep-Eagle
...
class CFBDataPreparation:
    """Prepare CFB data for Deep-Eagle LSTM training"""
    ...
    def get_opponent_stats(self, game_row, season, week):
        ...
        if len(opp_df) == 0:
            # No previous games, use defaults
            return {
                'opp_win_pct': 0.5,
                'opp_points_scored_avg': 25.0,
                'opp_points_allowed_avg': 25.0,
                'opp_total_yards_avg': 350.0
            }
        ...
        return {
            ...
            'opp_total_yards_avg': opp_df['total_yards'].mean() if opp_df['total_yards'].notna().any() else 350.0
        }
    ...
    def prepare_all_data(self, seasons=[2024, 2025]):
        """
            ...
            for team_id in team_ids:
                ...
                if team_df is not None and len(team_df) > 0:
                    ...
                    if teams_processed % 10 == 0:
                        print(f"  Processed {teams_processed}/{len(team_ids)} teams")
...
def main():
    """Prepare CFB data and save to file"""
    print("=" * 80)
    print("CFB DATA PREPARATION FOR DEEP-EAGLE")
    print("=" * 80)

    prep = CFBDataPreparation()

    # Prepare all data
    df = prep.prepare_all_data(seasons=[2024, 2025])

    # Remove rows with NaN values (from lag/rolling operations on first few games)
    print(f"\nCleaning data...")
    ...
    # Save to CSV
    output_file = 'cfb_training_data.csv'
    ...
    print(f"   Ready for Deep-Eagle training")
    print("=" * 80)
